...
class CustomerModel {
    ...
    // root query — tambah p_pred.platt_score_pct
    ...
    static async AddCustomerModel(customer_id, plan_id, contract_id, monthly_usage_hrs, feature_adoption_pct, payment_delay_count, support_ticket_last_90d, nps_score, tenure_months, last_login_days_ago, monthly_revenue, total_users) {
        try {
            ...
            if (exist) throw new NotFoundError(`Customer with id ${customer_id} already exist`);

            const textQuery = `
                INSERT INTO customers 
                    (customer_id, plan_id, contract_id, monthly_usage_hrs, feature_adoption_pct, payment_delay_count, support_ticket_last_90d, nps_score, tenure_months, last_login_days_ago, monthly_revenue, total_users) 
    ...
    static async UploadExcellCustomerModel(datas) {
        try {
            ...
            for (const data of datas) {
                ...
                if (exist) throw new NotFoundError(`Customer with id ${data.customer_id} already exist`);
                ...
                `;
                const values = [
                    data.customer_id, data.plan_id, data.contract_id,
                    data.monthly_usage_hrs, data.feature_adoption_pct, data.payment_delay_count,
                    data.support_ticket_last_90d, data.nps_score, data.tenure_months,
                    data.last_login_days_ago, data.monthly_revenue, data.total_users
                ];
    ...
    static async StatsCustomerByTypeModel() {
        const textQuery = `
            SELECT 
                ...
                ROUND((COUNT(c.id) * 100.0 / (SELECT COUNT(*) FROM customers)), 2) AS percentage
            FROM customers c
    ...
    static async StatsCustomerByChurnModel() {
        try {
            ...
            return {
                ...
                summary: { churn: totalChurn, not_churn: totalNotChurn },
                percentage: {
    ...
    static async PredictManualModel(inputData) {
        return new Promise((resolve, reject) => {
            ...
            pythonProcess.on('close', (code) => {
                ...
                reject(new Error(`Python Crash seketika (Code ${code}). Log: ${cleanError || 'Library pandas/joblib belum terinstal.'}`));
            });
        });
    }
    ...
    static async CheckCustomerIdExist(customer_id) {
        try {
            const textQuery = `SELECT 1 FROM customers WHERE customer_id = $1`;
